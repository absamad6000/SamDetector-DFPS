...
const App = (() => {
    ...
    function setupTheme() {
        ...
        // Load saved theme
        const savedTheme = localStorage.getItem('samdetect_theme') || 'dark';
        if (savedTheme === 'light') {
        ...
        themeBtn.addEventListener('click', () => {
            ...
            localStorage.setItem('samdetect_theme', newTheme);
            
            if (themeIcon) themeIcon.textContent = isLight ? '🌙' : '☀️';

            // Proactively re-render visualizations if data exists to pick up new colors
            ...
            if (state.similarityResult && state.similarityResult.validation) {
                Visualizations.renderValidation('validationPlot', state.similarityResult.validation);
            }
        });
    }
