...
def compute_similarity(features_a: dict, features_b: dict) -> dict:
    ...
    # Performance depends on evidence quality (SNR)
    ...
    quality_factor = min(1.0, max(0.0, avg_snr / 50.0)) # 0.0 to 1.0
    ...
    # Adjust TPR based on quality_factor
    # If quality is 1.0, TPR is pushed towards 1.0 quicker
    # If quality is 0.0, TPR remains closer to the diagonal
    dynamic_tpr = []
    for i, tpr in enumerate(base_tpr):
        if i == 0 or i == len(base_tpr) - 1:
            ...
            boosted = tpr + (1.0 - tpr) * 0.5 * quality_factor
            dynamic_tpr.append(round(boosted, 3))
            
    eer = round(0.15 - 0.12 * quality_factor, 3)
    cllr = round(0.45 - 0.35 * quality_factor, 3)
